...
const Item = ({task}) => {
  const style = task.completed ? {color: 'green', position: 'relative'} : {color: '', position: 'relative'};
  return (
    <li style={style} onClick={() => Actions.changeTaskStatus(task)}>
      <span>{task.name}</span>
      <button
        style={{float: 'right'}}
        onClick={e => {
...
const Actions = {
  ...
  removeTask(task) {
    initState.tasks = initState.tasks.filter(x => x.name !== task.name);
    initState = initState;
    render(App(initState), container);
  },
  addTask() {
